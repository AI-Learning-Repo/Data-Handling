...
# 35. Create Data with Potentially Unusual Transactions

Use the Seaborn `taxis` dataset. The exercise compares trip distance and fare, without treating unusual observations as confirmed fraud.

### Code Cell

```python id="7l2jwh"
taxis = sns.load_dataset("taxis").dropna(
    subset=["distance", "fare"]
)

X_anomaly_demo = taxis[["distance", "fare"]].to_numpy()
```
...
Some taxi trips may appear unusual because their distance and fare combination is far from the main pattern. Such observations are candidates for investigation, not automatically fraudulent transactions.
